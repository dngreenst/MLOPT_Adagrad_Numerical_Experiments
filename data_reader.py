...
class DataReader:
    ...
    def read(file: str, delimiter = None) -> np.array:
        ...
        columns = tuple([i for i in range(34)])
        kwargs = {'delimiter': delimiter, 'usecols': columns}
        return DataReader.numpy_loadtxt_memory_friendly(file, 1000000, usecols=columns, delimiter=delimiter)
